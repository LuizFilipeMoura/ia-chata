...
const WIRES = [
  ...
  { id: "grit", when: (c) => (c.state.game.gritTokens?.[c.side] || 0) > 0, text: () => `You're 2+ VP behind, so HQ sends a Grit token each round: a free face-down reaction that's Improved (tougher Brace, surer dodges, harder counter-hits), or an upgrade to one you already placed. Use it to break their hold.` },
