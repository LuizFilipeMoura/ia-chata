...
const WIRES = [
  ...
  { id: "escalation", when: (c) => (c.state.game.beaconMultiplier || 1) > 1, text: (c) => `Beacons now pay ×${c.state.game.beaconMultiplier}. They escalate (×1 in rounds 1-3, ×2 in 4-7, ×3 in 8-10), so a late push can overturn an early lead. Don't coast.` },
  { id: "grit", when: (c) => (c.state.game.gritTokens?.[c.side] || 0) > 0, text: () => `You're behind, so HQ sends Grit tokens each round (1 at 2+ VP behind, 2 at 5+, 3 at 8+): a free face-down reaction that's Improved (tougher Brace, surer dodges, harder counter-hits), or an upgrade to one you already placed. Use it to break their hold.` },
