...
`P(wound)` is boxed into **[10%, 90%]**, permanently. **Do not remove the clamp**: it is what guarantees no matchup is hopeless, and removing it reintroduces the 69
...
> did check. **When a claim goes false, re-derive it. Do not hedge it into vagueness**: > a vague sentence is unfalsifiable, which is worse than a wrong one.
...
- **Pen 7 vs Pen 6 is the reliability axis.** Siege Maul, Harpoon and Anchor sit in
  ...
  Wrecking Ball and Lance sit one point below it and **never saturate anything**: they bought alpha instead, and they roll for every wound they get.
