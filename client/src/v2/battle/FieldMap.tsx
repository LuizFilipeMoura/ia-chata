...
export function FieldMap({ field, objectives, mySide, ownerSide }: Props) {
  ...
  return (
    ...
    >
      ...
      <polygon className="v2-fm-half-mine" points={tri(mineC)} />
      <polygon className="v2-fm-half-foe" points={tri(foeC)} />
      <line className="v2-fm-diag" x1={sx(e0.x)} y1={sy(e0.y)} x2={sx(e1.x)} y2={sy(e1.y)} />

      <polygon className="v2-fm-deploy v2-fm-deploy--mine" points={deploySector(mineC)} />
      <polygon className="v2-fm-deploy v2-fm-deploy--foe" points={deploySector(foeC)} />

      {corners.map((c, i) => (
        <path
          key={`c${i}`} className="v2-fm-tick" fill="none"
          d={`M ${c.x + c.dx * T} ${c.y} L ${c.x} ${c.y} L ${c.x} ${c.y + c.dy * T}`}
        />
      ))}

      <text className="v2-fm-dim" x={PAD + fw / 2} y={PAD - 10} textAnchor="middle">
        {field.width}&#8243;
      </text>
      <text
        className="v2-fm-dim" x={PAD - 11} y={PAD + fh / 2} textAnchor="middle"
      ...
      <text className="v2-fm-zone-label" x={sx(mineLabel.x)} y={sy(mineLabel.y)} textAnchor="middle">
        You deploy
      </text>
      <text className="v2-fm-zone-label" x={sx(foeLabel.x)} y={sy(foeLabel.y)} textAnchor="middle">
        Enemy deploys
      </text>
    </svg>
  );
