...
export const GLOSSARY = [
  ...
  // ── Equipment tracked state (id-only lookup) ───────────────────────────────
  { id: "hardened", term: "Hardened", match: [],
    def: "Harden (Ablative Plating) is up: wound rolls against this Rig are at −1 Penetration (−2 with Reinforced Plating) until its next activation." },
  { id: "smoked", term: "Smoked", match: [],
    def: "Pop Smoke (Reactive Plating) is up: every attacker is at −2 accuracy against this Rig until its next activation, and missile Locks on it are broken." },
  { id: "overclocked", term: "Overclocked", match: [],
    def: "Overclock (Overclock Core) fired this activation: extra actions, paid for in heat." },
  { id: "reactor-overdrive", term: "Reactor Overdrive", match: [],
    def: "Overclock with Reactor Overdrive: +2 Penetration on every attack this activation, but the overheat bonus is doubled." },
  { id: "lock-sight", term: "Lock Sight", match: [],
    def: "Lock Sight (Targeting Computer): the next shot this activation rerolls all its missed to-hit dice." },
  { id: "primed-pen", term: "Primed Penetration", match: [],
    def: "Spent cryo or meltdown charge: bonus Penetration on this Rig's attacks this activation." },
  { id: "cryo", term: "Cryo", match: [],
    def: "Cryo Reservoir bank (max 3): spend N for free for −2 heat each and +N Penetration on the next attack. While any is banked, Recovery cools only 1." },
  { id: "meltdown", term: "Meltdown charge", match: [],
    def: "Meltdown Protocol bank (max 6), built from overheat instead of rolling. Spend N for free: +N Penetration, or N heat on every enemy within 4\". No venting or Shut Down while banked; it detonates if the Engine hits 0." },
  { id: "nanites", term: "Nanite stack", match: [],
    def: "Nanite Swarm: heals 1 SP on its location each Recovery, then decays 1 (max 3 per location). Heat Capacity −1 while any stack lives." },
  { id: "ablative-charges", term: "Ablative charges", match: [],
    def: "Ablative Cascade: each charge negates one landed wound for +1 heat. Refills to 2 each Recovery." },
  { id: "point-defense", term: "Point-Defense", match: [],
    def: "Point-Defense System: each interceptor forces an incoming ranged attack to reroll its hits, +1 heat each. Refills to 2 each Recovery; offline the round after the Rig fires its own ranged weapon." },
  { id: "fire-solution", term: "Fire solution", match: [],
    def: "Fire Solution Lock: each Fire at the same target stacks the solution (max 3); a full stack makes the next shot an auto-hit, armour-piercing volley. Switching target resets it." },
