...
3. **The chassis entry.** Add to `CHASSIS` in `shared/game-state.js`: `{ id, label, class, longRange, melee, sp: { hull, arms, legs, engine }, integrity }`. Class must be in `SUPPORTED_RIG_CLASSES` (light/medium). SP is per-chassis (~2× the class default), tuned to its durability tier. `integrity` is the whole-rig kill pool (§8a), start near `INTEGRITY_RATIO[class]` × total SP (light 0.5, medium 0.65): SP = how fast it gets crippled, Integrity = how fast it dies.
